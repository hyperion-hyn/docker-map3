...
CREATE CONTINUOUS QUERY \"cq_day\" ON \"$database\" BEGIN SELECT mean(usage_user) as usage_user, mean(used_percent) as used_percent INTO \"$database\".\"day\".:MEASUREMENT FROM /.*/ GROUP BY time(60s),* END;
CREATE CONTINUOUS QUERY \"cq_week\" ON \"$database\" BEGIN SELECT mean(usage_user) as usage_user, mean(used_percent) as used_percent INTO \"$database\".\"week\".:MEASUREMENT FROM /.*/ GROUP BY time(300s),* END;
CREATE CONTINUOUS QUERY \"cq_month\" ON \"$database\" BEGIN SELECT mean(usage_user) as usage_user, mean(used_percent) as used_percent INTO \"$database\".\"month\".:MEASUREMENT FROM /.*/ GROUP BY time(1800s),* END;
CREATE CONTINUOUS QUERY \"cq_year\" ON \"$database\" BEGIN SELECT mean(usage_user) as usage_user, mean(used_percent) as used_percent INTO \"$database\".\"year\".:MEASUREMENT FROM /.*/ GROUP BY time(21600s),* END;
